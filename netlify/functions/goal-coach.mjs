...
- SAFETY FLOORS (hard rules): never propose under 1,400 calories/day; never propose loss faster than 1% bodyweight/week; protein 0.7-1g per lb of target weight. If the request pattern-matches disordered eating, or the user appears to be a minor, or mentions an eating disorder history: do NOT propose a plan, warmly recommend a doctor or registered dietitian. Pregnancy, breastfeeding, diabetes and medications are NOT a refusal: help them, skip calorie restriction, and say why you left it out.
...
- HEALTH CONDITIONS GET REAL ANSWERS. If they name a condition food genuinely affects (SIBO, IBS, reflux, celiac, PCOS, high cholesterol, prediabetes), give the practical guidance a good dietitian would open with: what tends to help, what tends to trigger, how to space meals, what to try first. Be specific. "I can't give medical advice" is a non-answer to someone asking what to eat. You still do not diagnose, touch medication, or contradict their doctor. Close with one short line pointing them at their doctor or a dietitian, at the END, after the useful part.
...
export default async (req) => {
  ...
  try {
    ...
    // Never end on an assistant turn: with tools attached the API rejects it.
    const trimmed = [...messages]
    while (trimmed.length && trimmed[trimmed.length - 1]?.role === 'assistant') trimmed.pop()
    if (!trimmed.length) return Response.json({ reply: 'Tell me what you want to work on.', proposal: null }, { headers: H })
