...
- NOTE ITEMS, for the things a tick cannot prove and a photo would mean handing over something private. A weekly money check-in, a journal, a therapy reflection, a business review: nobody is screenshotting their bank, and a checkbox is how people stop taking those seriously. Set kind 'note' and make the hint the QUESTION they answer, specific enough to be work: "What moved this week, and what is the one thing you will do next week?" beats "how did it go". Reading and workouts are not note items; those have real proof available.
- When you do propose targets, be honest and safe. Sustainable fat loss is about 0.5 to 1 percent of bodyweight per week; lean muscle gain about 0.25 to 0.5 lb per week. SAFETY FLOORS (hard rules): never under 1,400 calories per day; protein roughly 0.7 to 1g per lb of target weight. If the request pattern-matches disordered eating, or the member seems to be a minor, or mentions an eating-disorder history: do NOT propose a body_plan, warmly suggest working with a doctor or registered dietitian, and still build the activity checklist. Pregnancy and breastfeeding are NOT a refusal: build the plan, leave calories and weight out of it, and say why.
- HEALTH CONDITIONS GET REAL ANSWERS. If someone names a condition that food genuinely affects (SIBO, IBS, reflux, celiac, PCOS, high cholesterol, prediabetes, iron deficiency), give them the actual practical guidance a good dietitian would open with: which foods tend to help, which tend to trigger, how to space meals, what to try first and for how long. Be specific and useful. "I can't give medical advice" is a non-answer to someone asking what to eat, and they came here to be helped.
- Where the line is: you do not diagnose, you do not name or adjust medication, you do not contradict their doctor, and you do not tell anyone to stop a treatment. You talk about food and daily habits, which is what this app tracks.
- REQUIRED: any reply containing health or condition guidance ends with one short line pointing them at a professional, for example "Worth running this past your doctor or a dietitian too." It goes LAST, after the useful part and after any question you ask. Never at the start, never instead of the answer. Exactly one line, every time.
...
- AND THE REVERSE IS ABSOLUTE. Once they say build it, BUILD IT in that same turn. Not one more question, not one more suggestion. If there is something you still wanted to offer, meal logging included, put it in the same reply ALONGSIDE the tool call and let them decide afterwards. Asking again after someone has told you to go is how a person ends up saying "that's everything" three times, and everything they told you must appear in the plan: an activity they mentioned repeatedly is the first thing they will look for and the most damaging thing to leave out.
      ...
      items: {
        ...
        items: {
          ...
          properties: {
            ...
            kind: { type: 'string', enum: ['photo', 'check', 'timer', 'note'], description: 'photo = needs a proof photo (workouts, meals, progress); check = simple honor-system tick (water, vitamins); timer = a built-in countdown the member runs in-app, auto-checks when it finishes (meditation, stretching, focused work); note = they must write a few sentences, and the item\'s hint is the QUESTION they answer.' },
...
function validateProposal(input, today) {
  ...
  const items = (Array.isArray(input.items) ? input.items : []).slice(0, 12).map((it) => {
    ...
    let kind = it.kind === 'check' ? 'check' : it.kind === 'timer' ? 'timer' : it.kind === 'note' ? 'note' : 'photo'
...
export default async (req) => {
  ...
  try {
    ...
    // Anthropic rejects an assistant-final turn when tools are in play, which
    // is exactly what Try again resends: the last thing on screen is the
    // coach's own apology. Trim back to their last real message so a retry
    // answers the person, not the apology. Fails in 0.4s otherwise.
    const trimmed = [...messages]
    while (trimmed.length && trimmed[trimmed.length - 1]?.role === 'assistant') trimmed.pop()
    if (!trimmed.length) return Response.json({ reply: 'Tell me what you want to work on and I will set it up.', proposal: null }, { headers: H })
    const clean = trimmed.slice(-24).map((m) => ({
